...
class ADMMDP:
    ...
    def allreduce_parameters(self):
        ...
        with torch.cuda.stream(self.dp_comm_stream):
            ...
            if self.flatten:
                ...
                self.dp_comm.all_reduce(self.flatten_para.data,
    ...
    def calculate_aux_grad(self):
        if self.synced:
            ...
            loss = 1e-4 * (self.flatten_para - self.z.data +
                           self.u.data).pow(2).sum()
    ...
    def profiling_data_parallel(self, init_time_stamp, init_event):
        ...
        if self.flatten:
            allreduce_slot = self.allreduce_gradients_start_event.elapsed_time(
                self.allreduce_grad_ready_event) * 1e+3
            allreduce_log = {
                "name": "opt_allreduce",
                "ph": "X",
            ...
            for name, para in self.module.named_parameters():
                allreduce_slot = self.allreduce_gradients_start_events[
                    name].elapsed_time(
                ...
                allreduce_log = {
                    ...
                    "tid": "7. optimizer-comm",
                    "ts":
                    self.get_ts(self.allreduce_gradients_start_events[name]),
                    "dur": allreduce_slot,
                    "cname": "cq_build_passed",
                    "args": {
                        'para': name,
