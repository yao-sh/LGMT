                ...
                for i_group, group in enumerate(
                    ...
                    for i_para, para in enumerate(group["params"]):
                        ...
                        comm_data_compressed_list = [
                            self._decompress(self._compress(x))
                            for x in comm_data_list
                        ]
                        comm_buffer_list = [
                            torch.zeros_like(x) for x in comm_data_list
                        ]

                        # revert
                        for i, _data_compressed in enumerate(
                                comm_data_compressed_list):
                        ...
                        server_data = sum(comm_buffer_list) / len(
                            comm_buffer_list)
                        server_data.add_(server_error[server_mask])
                        server_data_compressed = self._decompress(
                            self._compress(server_data))
                        server_error.data[server_mask] = (
                            server_data - server_data_compressed)
