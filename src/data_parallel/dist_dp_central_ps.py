...
class CentralPSDP:
        ...
        if self.enable_tidy_profiling:
            if name is None:
                self.dp_comm_stream.record_event(
                    self.reduce_gradients_start_event)
            else:
                self.dp_comm_stream.record_event(
    ...
    def _reduce_gradients(self):
        with torch.cuda.stream(self.dp_comm_stream):
            ...
            if self.flatten:
                ...
                self.dp_comm.reduce(self.flatten_para.grad,
                                    dst=0,
    ...
    def _broadcast_reduced_gradients(self):
        with torch.cuda.stream(self.dp_comm_stream):
            ...
            if self.flatten:
                ...
                self.dp_comm.broadcast(self.flatten_para.grad,
                                       src=0,
                                       stream=cupy_dp_stream)
                ...
                for name, para in self.module.named_parameters():
                    ...
                    self.dp_comm.broadcast(para.grad,
                                           src=0,
                                           stream=cupy_dp_stream)
    ...
    def profiling_data_parallel(self, init_time_stamp, init_event):
        ...
        profiling_log = []
        if self.flatten:
            reduce_slot = self.reduce_gradients_start_event.elapsed_time(
                self.reduce_gradients_end_event) * 1e+3
            reduce_log = {
                "name": "opt_reduce",
                "ph": "X",
            ...
            for name, para in self.module.named_parameters():
                reduce_slot = self.reduce_gradients_start_events[
                    name].elapsed_time(
                ...
                reduce_log = {
                    ...
                    "tid": "7. optimizer-comm",
                    "ts":
                    self.get_ts(self.reduce_gradients_start_events[name]),
                    "dur": reduce_slot,
                    "cname": "cq_build_passed",
                    "args": {
                        'para': name,
                        'size': torch.numel(para.data)
                    }
                }
                # print(reduce_log)
                profiling_log.append(reduce_log)
        ...
        if self.flatten:
            ...
            for name, para in self.module.named_parameters():
                broadcast_slot = self.broadcast_reduced_grad_start_events[
                    name].elapsed_time(
