    ...
    def __init__(self,
        ...
        if self.flatten:
            ...
            print("Flattened parameter grad number: {}, element size: {}.".
                  format(self.flatten_para.grad.numel(),
...
#             = self._declare_exp_avgs_buffer()
    ...
    def _declare_exp_avgs_buffer(self):
        ...
        bits = self.dp_bits
        _data = torch.zeros(chunk_size,
                            device=self.flatten_exp_avgs.device,
                            dtype=self.flatten_exp_avgs.dtype)
        #         _data_compressed = compress_flexible_nbits(_data, bits=bits, scale_dims=tuple())
        _data_compressed = compress_flexible_nbits_by_bucket(_data,
                                                             bits=bits,
                                                             bucket_size=512)
        grad_buffer = [(torch.zeros_like(_data_compressed[0]),
                        torch.zeros_like(_data_compressed[1]))
    ...
    def _declare_exp_avgs_buffer_by_data(self, data):
        ...
        #         _data_compressed = compress_flexible_nbits(_data, bits=bits, scale_dims=tuple())
        _data_compressed = compress_topk(_data,
                                         int(topk_ratio * _data.numel()))
        grad_buffer = [(torch.zeros_like(_data_compressed[0]),
                        torch.zeros_like(_data_compressed[1]))
                       for i in range(self.dp_group_size)]
        worker_errors = [
    ...
    def _sync_gradients(self):
        with torch.cuda.stream(self.dp_comm_stream):
            ...
            if self.flatten:
                ...
                self.dp_comm.all_reduce_opt_topk(self.flatten_para.grad,
                                                 stream=cupy_dp_stream)
                ...
                for name, para in self.module.named_parameters():
                    ...
                    if name not in self.buffers:
                        grad_buffer, worker_errors, server_error = self._declare_exp_avgs_buffer_by_data(
                            para.grad)
                        self.buffers[name] = (grad_buffer, worker_errors,
                                              server_error)
                    else:
                        grad_buffer, worker_errors, server_error = self.buffers[
                            name]

                    para.grad /= self.dp_group_size

                    self.dp_comm.all_reduce_opt_topk(
                        para.grad.view(-1),
                        buffer=grad_buffer,
    ...
    def _sync_exp_avgs(self):
        with torch.cuda.stream(self.dp_comm_stream):
            ...
            if flag.FLAG_DISABLE_COMPRESSION:
                # send grad
                self.dp_comm.all_reduce_opt(self.flatten_para.grad,
                                            self.grad_buffer_non_compressed,
                                            stream=cupy_dp_stream,
                                            caller=self)
                ...
                step_update_exp_avg(self.optimizer)
                self.dp_comm.all_reduce_opt_compressed(
                    self.flatten_exp_avgs.data,
                    self.exp_avg_buffer,
                    self.exp_avg_worker_errors,
                    self.exp_avg_server_error,
                    stream=cupy_dp_stream,
                    bits=self.dp_bits,
                    caller=self)
            self.profile_mark_allreduce_end()
