                    ...
                    for i_para, para in enumerate(group["params"]):
                        ...
                        if name not in dp_state_dict:
                            ...
                            for i in range(self.dp_group_size):
                                ...
                                # data to send
                                comm_data = torch.zeros(para.numel() //
                                                        self.dp_group_size,
                                                        dtype=torch.float16,
                                                        device=para.device)
                            ...
                            # server error
                            server_error = torch.zeros_like(
                                global_para.chunk(self.dp_group_size,
                                                  0)[self.dp_rank])
                        ...
                        for i in range(self.dp_group_size):
                            comm_data_list[i].data[:] = (
                                para[i * chunk_size:(i + 1) * chunk_size] -
