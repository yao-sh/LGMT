...
class ShardedPSDPCompressed:
    ...
    def __init__(self,
        ...
        print("Flattened parameter grad number: {}, element size: {}.".format(
            self.flatten_para.grad.numel(),
            self.flatten_para.grad.element_size()))

        exp_avgs = get_exp_avgs(self.optimizer)
        self.flatten_exp_avgs = flatten_tensors(exp_avgs,
                                                self.dp_group_size * 512 * 4)
    ...
    def _sync_gradients(self):
        with torch.cuda.stream(self.dp_comm_stream):
            ...
            if flag.FLAG_DISABLE_COMPRESSION:
                ...
                self.dp_comm.all_reduce_opt_compressed(self.flatten_para.grad,
                                                       self.grad_buffer,
                                                       self.worker_errors,
                                                       self.server_error,
                                                       stream=cupy_dp_stream,
