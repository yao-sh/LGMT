    ...
    for group in self.param_groups:
        for p in group["params"]:
            ...
            if len(state) == 0:
                ...
                state["exp_avg_global"] = torch.zeros_like(
                    p.data,
                    dtype=(torch.float16 if use_fp16 else torch.float32))
            ...
            exp_avg, exp_avg_sq, exp_avg_g = state["exp_avg"], state[
                "exp_avg_sq"], state["exp_avg_global"]
                ...
                if state["first"]:
                    ...
                    with torch.cuda.stream(dp_optimizer.dp_comm_stream):
                        cupy_dp_stream = cupy.cuda.ExternalStream(
                            dp_optimizer.dp_comm_stream.cuda_stream)
                        dp_optimizer.dp_comm_stream.record_event(
                            dp_optimizer.mmt_start_event)
                        exp_avg_g.data[:] = exp_avg
                        exp_avg_g.data /= dp_optimizer.dp_group_size
                        dp_optimizer.dp_comm.all_reduce(exp_avg_g.data,
                                                        stream=cupy_dp_stream)
                        dp_optimizer.dp_comm_stream.record_event(
                            dp_optimizer.mmt_ready_event)

                else:
                    print(f'sync... at {state["step"]}')
                    with torch.cuda.stream(dp_optimizer.dp_comm_stream):
                        dp_optimizer.dp_comm_stream.wait_event(
                            dp_optimizer.mmt_ready_event)
                        cupy_dp_stream = cupy.cuda.ExternalStream(
                            dp_optimizer.dp_comm_stream.cuda_stream)
                        dp_optimizer.dp_comm_stream.record_event(
