            ...
            with torch.cuda.stream(self.dp_comm_stream), cupy_dp_stream:
                ...
                if name not in dp_state_dict:
                    ...
                    dp_state_dict[name] = {
                        ...
                        global_para,
                        "worker_error": [
                            torch.zeros_like(server_error)
                            for xx in comm_data_list
                        ],
                        "server_error":
                        server_error,
                    }
                ...
                for i in range(self.dp_group_size):
                    ...
                    para.data[i * chunk_size:(i + 1) *
                              chunk_size][:] = global_para.data[i * chunk_size:
                                                                (i + 1) *
                                                                chunk_size]
                ...
                for i in range(self.dp_group_size):
                    for j, to_send in enumerate(comm_data_compressed_list[i]):
                        ...
                        if i != self.dp_rank:
                            call = self.dp_comm.isend(to_send,
                                                      dst=i,
                                                      stream=cupy_dp_stream)
                    ...
                    for to_recv in comm_buffer_list[i]:
                        ...
                        if i != self.dp_rank:
                            call = self.dp_comm.irecv(to_recv,
                                                      src=i,
                                                      stream=cupy_dp_stream)
                            _group_calls.append(call)
            ...
            with torch.cuda.stream(self.dp_comm_stream), cupy_dp_stream:
                ...
                for i_group, group in enumerate(
                    ...
                    for i_para, para in enumerate(group["params"]):
                        ...
                        print(
                            f'done first group r{self.global_rank} - {i_group}/{len(self.optimizer.optimizer.param_groups)} - {i_para}/{len(group["params"])} - {para.shape}'
                        )
                        self.dp_comm.barrier()
                        ...
                        print(
                            f'done second group r{self.global_rank} - {i_group}/{len(self.optimizer.optimizer.param_groups)} - {i_para}/{len(group["params"])} - {para.shape}'
                        )
                        self.dp_comm.barrier()
