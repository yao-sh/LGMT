    ...
    def _partial_sync(self):
        ...
        if self.flatten:
            ...
            with torch.cuda.stream(self.dp_comm_stream), cupy_dp_stream:
                ...
                server_mask = comm_mask_list[self.dp_rank].to(
                    server_error.device)
                ...
                del comm_data_list
                comm_buffer_list = [[
                    torch.zeros_like(x, device='cpu') for x in x_tuple
                ...
                server_data.add_(server_error[server_mask].to(
                    server_data.device))
                server_data_compressed, server_data_meta = self._compress(
                    server_data)
                server_error.data[server_mask] = (
                    server_data - self._decompress(server_data_compressed,
                                                   server_data_meta)).to(
                                                       server_error.device)
            ...
            with torch.cuda.stream(self.dp_comm_stream), cupy_dp_stream:
                ...
                for i_group, group in enumerate(
                    ...
                    for i_para, para in enumerate(group["params"]):
                        ...
                        server_error.data[server_mask] = (
                            server_data -
                            self._decompress(server_data_compressed,
                                             server_data_meta)).cpu()
