...
import nltk
import spacy
from helm.common.general import ensure_file_downloaded, match_case
...
class SynonymPerturbation(Perturbation):
    ...
    **Output:**
    ...
    def __init__(self, prob: float):
        ...
        output_dir = os.path.join("benchmark_output", "perturbations",
                                  self.name)
        ...
        ensure_file_downloaded(source_url=self.SOURCE_URI,
    ...
    def description(self) -> PerturbationDescription:
        return SynonymPerturbation.Description(name=self.name,
                                               robustness=True,
                                               prob=self.prob)
    ...
    def perturb(self, text: str, rng: Random) -> str:
        ...
        for token in doc:
            ...
            synonyms = []
            if wordnet_pos:
                for base in wordnet._morphy(word.lower(
                ), wordnet_pos):  # _morphy returns the base form of a word
                    synonyms.extend(
                        self.wordnet_synonyms.get(f"{base}:{wordnet_pos}", []))
            synonyms = [s for s in synonyms if s != word.lower()]
            synonyms = list(dict.fromkeys(
                synonyms))  # Make the list unique while preserving the order
