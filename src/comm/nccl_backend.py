...
class NCCLCommunicator:
    ...
    def __init__(self, comm_rank: int, cuda_id: int, comm_group_size: int,
                 comm_name: str):
        self.comm_rank = comm_rank
        cupy.cuda.Device(cuda_id).use()
        self.comm_group_size = comm_group_size
        print("Initialize NCCLCommunicator: <", comm_name, ">; rank:",
              comm_rank)
        ...
        if self.comm_rank == 0:
            ...
            self.dist_store.set('group-' + comm_name + '-unique-id',
                                cuda_id_str)
            # print("Master put <group-"+comm_name+"-unique-id: ", cuda_id_str, ">.")
        else:
            cuda_id_str = self.dist_store.get('group-' + comm_name +
                                              '-unique-id')

        comm_id = tuple(np.frombuffer(cuda_id_str, dtype=int))
        # comm_id = cupy.cuda.nccl.get_unique_id()
        # print(comm_id)
        self.comm = cupy.cuda.nccl.NcclCommunicator(comm_group_size, comm_id,
                                                    comm_rank)
    ...
    def send(self,
        ...
        self.comm.send(tensor.data_ptr(), torch.numel(tensor),
                       _type_torch_to_cupy(tensor.dtype), dst, stream.ptr)
    ...
    def recv(self,
        ...
        self.comm.recv(tensor.data_ptr(), torch.numel(tensor),
                       _type_torch_to_cupy(tensor.dtype), src, stream.ptr)

    def broadcast(self,
                  tensor: torch.Tensor,
                  src: int,
                  stream=cupy.cuda.Stream.null):
        self.comm.bcast(tensor.data_ptr(), torch.numel(tensor),
                        _type_torch_to_cupy(tensor.dtype), src, stream.ptr)
    ...
    def all_reduce(self,
                   ...
                   stream=cupy.cuda.Stream.null,
                   op=cupy.cuda.nccl.NCCL_SUM):
        self.comm.allReduce(tensor.data_ptr(), tensor.data_ptr(),
                            torch.numel(tensor),
                            _type_torch_to_cupy(tensor.dtype), op, stream.ptr)
    ...
    def all_to_all(self,
                   ...
                   stream=cupy.cuda.Stream.null):
        assert len(output_tensor_list) == self.comm_group_size and len(
            input_tensor_list) == self.comm_group_size
        cupy.cuda.nccl.groupStart()
        for i in range(self.comm_group_size):
    ...
    def all_reduce_opt(self,
        ...
        cupy.cuda.nccl.groupStart()
        for i in range(self.comm_group_size):
            self.comm.send(tensor.data_ptr() + i * chunk_size * element_size,
                           chunk_size, t_type, i, stream.ptr)
            self.comm.recv(buffer[i].data_ptr(), chunk_size, t_type, i,
                           stream.ptr)
        cupy.cuda.nccl.groupEnd()
...
#         caller.dp_comm_stream.record_event(caller.sync_start_event)
        cupy.cuda.nccl.groupStart()
        for i in range(self.comm_group_size):
            self.comm.send(buffer[0].data_ptr(), chunk_size, t_type, i,
                           stream.ptr)
            self.comm.recv(tensor.data_ptr() + i * chunk_size * element_size,
                           chunk_size, t_type, i, stream.ptr)
        cupy.cuda.nccl.groupEnd()
...
#                 _data, bits=bits, scale_dims=tuple()) for _data in tensor_chunks]
            tensor_chunks_compressed = [
                compress_flexible_nbits_by_bucket(_data,
                                                  bits=bits,
                                                  bucket_size=128)
                for _data in tensor_chunks
            ]

            # update worker errors
            for i in range(self.comm_group_size):
                worker_errors[i].set_(
                    (tensor_chunks[i] - decompress_flexible_nbits_by_bucket(
                        *tensor_chunks_compressed[i],
                        bits=bits,
                        original_shape=original_shape,
                        bucket_size=128)).type(worker_errors[i].dtype))
            del tensor_chunks
            ...
            for i in range(self.comm_group_size):
                ...
                self.comm.send(to_send.data_ptr(), to_send.numel(),
                               _type_torch_to_cupy(to_send.dtype), i,
                               stream.ptr)
                ...
                self.comm.send(to_send.data_ptr(), to_send.numel(),
                               ...
                               stream.ptr)
                to_recv = buffer[i][0]
                self.comm.recv(to_recv.data_ptr(), to_recv.numel(),
                               _type_torch_to_cupy(to_recv.dtype), i,
                               stream.ptr)
                to_recv = buffer[i][1]
                self.comm.recv(to_recv.data_ptr(), to_recv.numel(),
                               _type_torch_to_cupy(to_recv.dtype), i,
            ...
            tensor_server_compressed = compress_flexible_nbits_by_bucket(
                tensor_server, bits=bits, bucket_size=128)
            ...
            server_error.set_(
                (tensor_server - decompress_flexible_nbits_by_bucket(
                    *tensor_server_compressed,
                    bits=bits,
                    original_shape=original_shape,
                    bucket_size=128)).type(server_error.dtype))
            ...
            for i in range(self.comm_group_size):
                self.comm.send(
                    tensor_server_compressed[0].data_ptr(),
                    tensor_server_compressed[0].numel(),
                    _type_torch_to_cupy(tensor_server_compressed[0].dtype), i,
                    stream.ptr)
                self.comm.send(
                    tensor_server_compressed[1].data_ptr(),
                    tensor_server_compressed[1].numel(),
                    _type_torch_to_cupy(tensor_server_compressed[1].dtype), i,
                    stream.ptr)
    ...
    def all_reduce_opt_topk(self,
        ...
        with stream:
            ...
            # decompress
            tensor_chunks_compressed = [
                compress_topk(_data, int(topk_ratio * _data.numel()))
                for _data in tensor_chunks
            ]
            ...
            for i in range(self.comm_group_size):
                worker_errors[i].set_(
                    (tensor_chunks[i] -
                     decompress_topk(*tensor_chunks_compressed[i],
                                     original_shape=original_shape)).type(
                                         worker_errors[i].dtype))
            del tensor_chunks
            #             caller.dp_comm_stream.record_event(caller.worker_compress_end_event)
            ...
            tensor_server_compressed = compress_topk(
                tensor_server, int(topk_ratio * tensor_server.numel()))

            # update server error
            server_error.set_(
                (tensor_server -
                 decompress_topk(*tensor_server_compressed,
                                 original_shape=original_shape)).type(
                                     server_error.dtype))
            #             caller.dp_comm_stream.record_event(caller.server_compress_end_event)
    ...
    def all_reduce_opt_topk_2(self,
        ...
        with stream:
            ...
            tensor = (data.to(global_data.dtype) -
                      global_data) / self.comm_group_size  # gradient
            ...
            # revert back
            for i in range(self.comm_group_size):
                data.data[i * original_shape[0]:(i + 1) *
                          original_shape[0]] -= decompress_topk(
                              *tensor_chunks_compressed[i],
                              original_shape=original_shape).to(data.dtype)
...
#             caller.dp_comm_stream.record_event(caller.gather_start_event)
            ...
            updated = (tensor != 0)
            print(updated.sum() / updated.numel(),
