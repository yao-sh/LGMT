...
class TorchCommunicator:
    ...
    def send(self, tensor: torch.Tensor, dst: int, stream=None):
        ...
        if tensor.device == torch.device('cpu'):
            dist.send(tensor,
                      self.to_global_rank(dst),
                      group=self.process_group)
        else:
            dist.send(tensor.cpu(),
                      self.to_global_rank(dst),
                      group=self.process_group)
    ...
    def isend(self, tensor: torch.Tensor, dst: int, stream=None):
        # print("Send tensor of size:", torch.numel(tensor))
        if tensor.device == torch.device('cpu'):
            handler = dist.isend(tensor,
                                 self.to_global_rank(dst),
                                 group=self.process_group)
        else:
            handler = dist.isend(tensor.cpu(),
                                 self.to_global_rank(dst),
                                 group=self.process_group)
        return handler
    ...
    def irecv(self, tensor: torch.Tensor, src: int, stream=None):
        ...
        if tensor.device == torch.device('cpu'):
            handler = dist.irecv(tensor,
                                 self.to_global_rank(src),
                                 group=self.process_group)
        else:
            assert False
            buffer = tensor.cpu()
            handler = dist.irecv(buffer,
                                 self.to_global_rank(src),
                                 group=self.process_group)
            tensor[:] = buffer.to(tensor.device)
        return handler
            ...
            for i in range(self.comm_group_size):
                for j, to_send in enumerate(tensor_chunks_compressed[i]):
                    if i != self.dp_rank:
                        ...
                        call = self.isend(to_send, dst=i, stream=stream)
                ...
                for to_recv in buffer[i]:
                    if i != self.dp_rank:
                        ...
                        call = self.irecv(to_recv, src=i, stream=stream)
                        _group_calls.append(call)
    ...
    def all_reduce_opt_topk(self,
        ...
        with stream:
            ...
            tensor_server = decompress_topk(
                buffer[0][0].to(tensor.device),
                buffer[0][1].to(tensor.device),
                original_shape=original_shape,
            )
            for i in range(1, self.comm_group_size):
                tensor_server += decompress_topk(
                    buffer[i][0].to(tensor.device),
                    buffer[i][1].to(tensor.device),
                    original_shape=original_shape,
                )
            # tensor_server.mul_(1 / self.comm_group_size)
...
def default_init(args):
    if hasattr(args, 'world_size'):
        ...
        dist.init_process_group(backend='nccl',
                                init_method=args.dist_url,
                                world_size=args.pipeline_group_size,
                                rank=args.rank)
...
def init_with_coordinator(args, prime_ip, rank, port=9999):
    if hasattr(args, 'world_size'):
        dist.init_process_group(backend='gloo',
                                init_method='tcp://' + prime_ip + f':{port}',
                                world_size=args.world_size,
                                rank=rank)
    else:
        assert hasattr(args, 'pipeline_group_size')
