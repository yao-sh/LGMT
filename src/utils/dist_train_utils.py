...
def distributed_train_foo_iter(args, pipeline, device, train_data_loader):
    ...
    if get_pipeline_parallel_rank() == 0:
        ...
        print("Finished running ", args.num_iters,
              " iterations, averaged (exclude the first iter) run time:",
              averaged_time)
...
def distributed_train_lm_iter(args, pipeline, device, train_data_loader):
    ...
    if get_pipeline_parallel_rank() == 0:
        total_time = 0
        for i, (data_previous, data,
                data_next) in enumerate(previous_and_next(train_data_loader)):
            input_ids = data['text'].to(device)
            data_ids = (
                data_previous['idx'] if data_previous is not None else None,
                data['idx'],
                data_next['idx'] if data_next is not None else None,
            )
            current_iter_time = pipeline.sgd_iter(input_ids, None, data_ids)
            if i > 0:
                total_time += current_iter_time
...
def distributed_train_bert_iter(args, pipeline, device, train_data_loader):
    ...
    if get_pipeline_parallel_rank() == 0:
        ...
        for i, (data_previous, data,
            ...
            current_iter_time = pipeline.sgd_iter(inputs_ids,
                                                  None,
                                                  data_ids,
