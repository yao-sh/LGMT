...
def add_torch_distributed_arguments(parser):
    ...
    parser.add_argument('--dp-backend',
                        type=str,
                        default='gloo',
                        metavar='S',
    ...
    parser.add_argument('--pipeline-group-size',
                        type=int,
                        default=4,
                        metavar='D',
                        help='world-size (default: 2)')
    parser.add_argument('--data-group-size',
                        type=int,
                        default=1,
                        metavar='D',
...
def add_task_arguments(parser):
    ...
    parser.add_argument(
        ...
        metavar='S',
        help='which tokenizer to use.')
    parser.add_argument('--vocab-extra-ids',
                        type=int,
                        default=0,
                        metavar='N',
                        help='-')
...
def add_mixed_precision_arguments(parser):
    ...
    parser.add_argument('--initial-loss-scale',
                        type=float,
                        default=32768,
                        help='Initial loss-scale for dynamic loss scaling.')
    parser.add_argument('--min-loss-scale',
                        type=float,
                        default=1.0,
                        help='Minimum loss scale for dynamic loss scale.')
    parser.add_argument('--loss-scale-window',
                        type=float,
                        default=1000,
    ...
    parser.add_argument('--use-offload',
                        action='store_true',
...
def add_parallel_schema_arguments(parser):
    ...
    parser.add_argument(
        ...
        default=1,
        help=
        'Number of gradient computation in Pipeline without data parallel sync.'
    )


def add_acitvation_compression_arguments(parser):
    parser.add_argument('--forward-compress-method',
                        type=str,
                        default='none',
                        metavar='N',
                        help='-')
    parser.add_argument('--forward-bits',
                        type=int,
                        default=4,
                        metavar='N',
                        help='-')
    parser.add_argument('--forward-bits-act',
                        type=int,
                        default=4,
                        metavar='N',
                        help='only for <delta-lowbits>')
    parser.add_argument('--forward-ratio',
                        type=float,
                        default=1.0,
                        metavar='N',
                        help='-')
    parser.add_argument('--forward-ratio-act',
                        type=float,
                        default=1.0,
                        metavar='N',
                        help='-')
    parser.add_argument('--forward-scale-method',
                        type=str,
                        default='max',
                        metavar='N',
                        help='-')
    parser.add_argument('--forward-scale-dims',
                        type=lambda x: tuple(
                            int(i)
                            for i in x.lstrip('(').rstrip(')').split(',')
                            if i.strip() != ''),
                        default=(0, 1),
                        metavar='N',
                        help='-')
    parser.add_argument('--backward-compress-method',
                        type=str,
                        default='none',
                        metavar='N',
                        help='-')
    parser.add_argument('--backward-bits',
                        type=int,
                        default=8,
                        metavar='N',
                        help='-')
    parser.add_argument('--backward-ratio',
                        type=float,
                        default=1.0,
                        metavar='N',
                        help='-')
    parser.add_argument('--backward-scale-method',
                        type=str,
                        default='max',
                        metavar='N',
                        help='-')
    parser.add_argument('--backward-scale-dims',
                        type=lambda x: tuple(
                            int(i)
                            for i in x.lstrip('(').rstrip(')').split(',')
                            if i.strip() != ''),
