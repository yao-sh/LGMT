...
def load_checkpoint(pipe, args):
    ...
    checkpoint_step_path = os.path.join(args.checkpoint_path,
                                        f"checkpoint_{latest_step}")
...
def save_checkpoint(pipe, args):
    ...
    torch.save(
        ...
        os.path.join(checkpoint_step_path,
                     f'prank_{get_pipeline_parallel_rank()}_checkpoint.pt'))
...
def save_stream_dataloader_state_dict(dataloader, pipe, args):
    ...
    torch.save(dataloader.dataset.state_dict(),
               os.path.join(checkpoint_step_path, f'dataset_state_dict.pt'))
