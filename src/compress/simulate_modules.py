...
class SimulateTestCompression:
    ...
    def build_buffer(self,
        ...
        self.cache = np.memmap(
            ...
            mode='w+',
            dtype=np.float16,
            shape=(MAX_CACHE_SIZE, seq_length, embedding_dim),
        )
        ...
        self.ccache = torch.randn([int(1e4), embedding_dim],
                                  dtype=dtype,
                                  device=device).half() / 100.
    ...
    def compress(self, x):
        with torch.no_grad():
            ...
            if self.count < self.ccache.size(0):
                ...
                cache_idx = faiss.knn_gpu(self.res,
                                          x_,
                                          self.ccache,
                                          1,
                                          metric=faiss.METRIC_L1)[1]
            ...
            if flag.FLAG_DISABLE_COMPRESSION:
                ...
                self.ccache.data[cache_idx] = self.ccache.data[
                    cache_idx] * 0.9 + 0.1 * x_to_save.half()
                return x

            x_hat = self.ccache[cache_idx]
            delta = x - x_hat
            compressed_delta = compress_nbits(delta,
                                              self.bits,
...
class SimulateTestCompressionLR:
    ...
    def build_buffer(self,
        ...
        self.cache = torch.randn(
            [int(1e5), embedding_dim], dtype=dtype, device=device) / 100.
        self.count = 0
...
class SimulateDeltaCompression:
    ...
    def build_buffer(self,
        ...
        self.simulate_cache = np.zeros(
            (MAX_CACHE_SIZE, seq_length, embedding_dim),
            dtype=np.float32,
        )
    ...
    def simulate_kmeans(self):
        ...
        if hasattr(self, 'kmeans'):
            ...
            init_centroids = None
        self.kmeans = faiss.Kmeans(d=self.cache.shape[-1],
                                   k=int(1e5),
                                   niter=1,
                                   nredo=1)
        data_train = self.cache[:, 1].reshape(
            -1, self.cache.shape[-1]).astype('float32')
        data_train /= (np.linalg.norm(data_train, axis=1, keepdims=True) +
                       1e-8)
        self.kmeans.train(
            data_train,
    ...
    def simulate_cluster_compress(self, x, i_micro_batch):
        sample_ids = self.sample_ids[i_micro_batch *
                                     self.micro_batch_size:(i_micro_batch +
                                                            1) *
                                     self.micro_batch_size]
