...
class DeltaCompressor:
    ...
    def build_buffer(self,
        ...
        self.cache = np.memmap(
            ...
            mode='w+',
            dtype=np.float16,
            shape=(MAX_CACHE_SIZE, 2, seq_length, embedding_dim),
        )
        ...
        _a, _s = compress_flexible_nbits(_x,
                                         self.bits,
                                         scale_method=self.scale_method,
                                         scale_dims=self.scale_dims)
        self.buffers = [(
            torch.zeros(_a.shape,
                        requires_grad=False,
                        device=device,
                        dtype=torch.uint8),
            torch.zeros(_s.shape,
                        requires_grad=False,
                        device=device,
                        dtype=torch.float16),
        ...
        self.warmup_buffers = [
            torch.zeros(
                (micro_batch_size, seq_length, embedding_dim),
                requires_grad=False,
                device=device,
                dtype=dtype,
            ) for _ in range(batch_size // micro_batch_size)
        ...
        # CPU RAM Buffers
        self.np_dec_buffers = [
            pin_memory(np.zeros(self.activ_shape, dtype=np.float16))
            for _ in range(batch_size // micro_batch_size)
        ]
        self.np_com_buffers = [
            pin_memory(np.zeros(self.activ_shape, dtype=np.float16))
        ...
        # GPU RAM Buffers
        self.cp_dec_buffers = [
            cupy.empty(self.activ_shape, dtype=np.float16)
            for _ in range(batch_size // micro_batch_size)
        ]
        self.cp_com_buffers = [
            cupy.empty(self.activ_shape, dtype=np.float16)
    ...
    def _read_from_cache(self, sample_ids):
        ...
        for i in range(self.batch_size // self.micro_batch_size):
            self.np_dec_buffers[i][:] = a_dec[i *
                                              self.micro_batch_size:(i + 1) *
                                              self.micro_batch_size]
            self.np_com_buffers[i][:] = a_com[i *
                                              self.micro_batch_size:(i + 1) *
                                              self.micro_batch_size]
    ...
    def read_from_cache(self, sample_ids):
        self._wait_read()
        self._wait_write()
        self.future_read = self.executor.submit(self._read_from_cache,
                                                sample_ids)

    def write_to_cache(self, sample_ids):
        self._wait_read()
        self._wait_write()
        self.future_write = self.executor.submit(self._write_to_cache,
                                                 sample_ids)
    ...
    def compress(self, x, i_micro_batch):
        # get cache
        self.cp_com_buffers[i_micro_batch].set(
            self.np_com_buffers[i_micro_batch])
        ...
        compressed_delta = compress_flexible_nbits(
            ...
            scale_dims=self.scale_dims)
        # update cache
        delta = decompress_flexible_nbits(*compressed_delta, self.bits,
                                          self.activ_shape)
        x = last_x + delta
    ...
    def decompress(self, delta, i_micro_batch):
        # get cache
        self.cp_dec_buffers[i_micro_batch].set(
            self.np_dec_buffers[i_micro_batch])
    ...
    def recv_decompress(self, i_micro_batch, comm, src, stream):
        ...
        if not flag.FLAG_DISABLE_COMPRESSION:
            ...
            with stream:
                x = self.no_decompress(recv_buffer,
                                       i_micro_batch=i_micro_batch)
            return x
...
class DeltaLowBitsCompressor(DeltaCompressor):
    ...
    def build_buffer(self,
        ...
        # CPU RAM Buffers
        self.micro_compressed_activ_shape = (self.micro_batch_size,
                                             *self.compressed_activ_shape)
        self.micro_compressed_scale_shape = (self.micro_batch_size,
                                             *self.compressed_scale_shape)
        self.np_dec_activ_buffers = [
            pin_memory(
                np.zeros(self.micro_compressed_activ_shape, dtype=np.uint8))
            for _ in range(batch_size // micro_batch_size)
        ]
        self.np_dec_scale_buffers = [
            pin_memory(
                np.zeros(self.micro_compressed_scale_shape, dtype=np.float16))
            for _ in range(batch_size // micro_batch_size)
        ]
        ...
        self.cp_dec_activ_buffers = [
            cupy.empty(self.micro_compressed_activ_shape, dtype=np.uint8)
            for _ in range(batch_size // micro_batch_size)
        ]
        self.cp_dec_scale_buffers = [
            cupy.empty(self.micro_compressed_scale_shape, dtype=np.float16)
            for _ in range(batch_size // micro_batch_size)
        ]
    ...
    def _read_from_cache(self, sample_ids):
        ...
        for i in range(self.batch_size // self.micro_batch_size):
            self.np_dec_activ_buffers[i][:] = a_dec[i *
                                                    self.micro_batch_size:(i +
                                                                           1) *
                                                    self.micro_batch_size]
            self.np_com_activ_buffers[i][:] = a_com[i *
                                                    self.micro_batch_size:(i +
                                                                           1) *
                                                    self.micro_batch_size]
            self.np_dec_scale_buffers[i][:] = s_dec[i *
                                                    self.micro_batch_size:(i +
                                                                           1) *
                                                    self.micro_batch_size]
            self.np_com_scale_buffers[i][:] = s_com[i *
                                                    self.micro_batch_size:(i +
                                                                           1) *
                                                    self.micro_batch_size]
    ...
    def compress(self, x, i_micro_batch):
        # get cache
        self.cp_com_activ_buffers[i_micro_batch].set(
            self.np_com_activ_buffers[i_micro_batch])
        self.cp_com_scale_buffers[i_micro_batch].set(
            self.np_com_scale_buffers[i_micro_batch])
        last_compressed_activ = cupy_to_tensor(
            self.cp_com_activ_buffers[i_micro_batch])
        last_compressed_scale = cupy_to_tensor(
            self.cp_com_scale_buffers[i_micro_batch])
        last_x = decompress_nbits(last_compressed_activ,
                                  last_compressed_scale,
                                  bits=self.bits_act)
        # compresss delta
        delta = x - last_x
        compressed_delta = compress_flexible_nbits(
    ...
    def decompress(self, delta, i_micro_batch):
        # get cache
        self.cp_dec_activ_buffers[i_micro_batch].set(
            self.np_dec_activ_buffers[i_micro_batch])
        self.cp_dec_scale_buffers[i_micro_batch].set(
            self.np_dec_scale_buffers[i_micro_batch])
        last_compressed_activ = cupy_to_tensor(
            self.cp_dec_activ_buffers[i_micro_batch])
        last_compressed_scale = cupy_to_tensor(
            self.cp_dec_scale_buffers[i_micro_batch])
        last_x = decompress_nbits(last_compressed_activ,
                                  last_compressed_scale,
                                  bits=self.bits_act)
        # decompress delta
...
def _compress_topk_nbits(x, k, bits, scale_method='max'):
    ...
    # 2. quantize x
    uint_x, scales = _compress_nbits(x,
                                     bits,
    ...
    masks = cupy_to_tensor(cupy.packbits(tensor_to_cupy(masks))).view(
        batch_size, -1)
    return topk_uint_x, masks, scales


def _decompress_topk_nbits(topk_uint_x, masks, scales, k, bits,
                           original_shape):
    masks = cupy_to_tensor(cupy.unpackbits(tensor_to_cupy(masks)))
    ...
    if bits == 8:
        ...
        raise Exception('not support bits')
    uint_x = torch.zeros(original_shape,
                         dtype=torch.uint8,
                         device=topk_uint_x.device) + (1 << bits - 1)
...
class DeltaTopKLowBitsCompressor(DeltaCompressor):
    ...
    def build_buffer(self,
        ...
        self.k_act = self.k_act + ((8 // self.bits_act) - self.k_act %
                                   (8 // self.bits_act))
        ...
        self.micro_compressed_activ_shape = (self.micro_batch_size,
                                             *self.compressed_activ_shape)
        self.micro_compressed_masks_shape = (self.micro_batch_size,
                                             *self.compressed_masks_shape)
        self.micro_compressed_scale_shape = (self.micro_batch_size,
                                             *self.compressed_scale_shape)
        self.np_dec_activ_buffers = [
        ...
        self.np_dec_masks_buffers = [
            pin_memory(
                np.zeros(self.micro_compressed_masks_shape, dtype=np.uint8))
            for _ in range(batch_size // micro_batch_size)
        ]
        self.np_dec_scale_buffers = [
            pin_memory(
        ...
        self.cp_dec_masks_buffers = [
            cupy.empty(self.micro_compressed_masks_shape, dtype=np.uint8)
            for _ in range(batch_size // micro_batch_size)
        ]
        self.cp_dec_scale_buffers = [
            cupy.empty(self.micro_compressed_scale_shape, dtype=np.float16)
        ...
        for i in range(self.batch_size // self.micro_batch_size):
            ...
            self.np_com_activ_buffers[i][:] = a_com[i *
                                                    self.micro_batch_size:(i +
                                                                           1) *
                                                    self.micro_batch_size]
            self.np_dec_masks_buffers[i][:] = m_dec[i *
                                                    self.micro_batch_size:(i +
                                                                           1) *
                                                    self.micro_batch_size]
            self.np_com_masks_buffers[i][:] = m_com[i *
                                                    self.micro_batch_size:(i +
                                                                           1) *
                                                    self.micro_batch_size]
            self.np_dec_scale_buffers[i][:] = s_dec[i *
    ...
    def compress(self, x, i_micro_batch):
        ...
        self.cp_com_activ_buffers[i_micro_batch].set(
            self.np_com_activ_buffers[i_micro_batch])
        self.cp_com_masks_buffers[i_micro_batch].set(
            self.np_com_masks_buffers[i_micro_batch])
        self.cp_com_scale_buffers[i_micro_batch].set(
            self.np_com_scale_buffers[i_micro_batch])
        last_compressed_activ = cupy_to_tensor(
            self.cp_com_activ_buffers[i_micro_batch])
        last_compressed_masks = cupy_to_tensor(
            self.cp_com_masks_buffers[i_micro_batch])
        last_compressed_scale = cupy_to_tensor(
            self.cp_com_scale_buffers[i_micro_batch])
        last_x = _decompress_topk_nbits(
            last_compressed_activ,
            last_compressed_masks,
            last_compressed_scale,
            bits=self.bits_act,
            k=self.k_act,
            original_shape=self.activ_shape,
        )
        # compresss delta
        delta = x - last_x
        compressed_delta = compress_flexible_nbits(
        ...
        delta = decompress_flexible_nbits(*compressed_delta, self.bits,
                                          self.activ_shape)
        x = last_x + delta
        compressed_x = _compress_topk_nbits(x,
                                            bits=self.bits_act,
                                            k=self.k_act,
                                            scale_method='max')
        a_cp = tensor_to_cupy(compressed_x[0])
    ...
    def decompress(self, delta, i_micro_batch):
        ...
        self.cp_dec_activ_buffers[i_micro_batch].set(
            self.np_dec_activ_buffers[i_micro_batch])
        self.cp_dec_masks_buffers[i_micro_batch].set(
            self.np_dec_masks_buffers[i_micro_batch])
        self.cp_dec_scale_buffers[i_micro_batch].set(
            self.np_dec_scale_buffers[i_micro_batch])
        last_compressed_activ = cupy_to_tensor(
            self.cp_dec_activ_buffers[i_micro_batch])
        last_compressed_masks = cupy_to_tensor(
            self.cp_dec_masks_buffers[i_micro_batch])
        last_compressed_scale = cupy_to_tensor(
            self.cp_dec_scale_buffers[i_micro_batch])
        last_x = _decompress_topk_nbits(
            last_compressed_activ,
...
def _compress_topk_nbits_batch(x, k, bits, scale_method='max'):
    ...
    if bits == 8:
        ...
        raise Exception('not support bits')
    masks = cupy_to_tensor(cupy.packbits(tensor_to_cupy(masks))).view(-1)
    return topk_uint_x, masks, scales


def _decompress_topk_nbits_batch(topk_uint_x, masks, scales, k, bits,
                                 original_shape):
    masks = cupy_to_tensor(cupy.unpackbits(tensor_to_cupy(masks)))
    masks = masks.view(original_shape)
    if bits == 8:
...
class TopKDeltaCompressor(DeltaCompressor):
    ...
    def build_buffer(self,
        ...
        self.micro_batch_size = micro_batch_size
        self.activ_shape = (micro_batch_size, seq_length, embedding_dim)
        self.k_delta = int(micro_batch_size * seq_length * embedding_dim *
                           self.ratio)
        self.k_delta = self.k_delta + ((8 // self.bits) - self.k_delta %
                                       (8 // self.bits))
        ...
        self.buffers = [(
            torch.zeros(self.compressed_delta_shape,
                        requires_grad=False,
                        device=device,
                        dtype=torch.uint8),
            torch.zeros(self.compressed_masks_shape,
                        requires_grad=False,
                        device=device,
                        dtype=torch.uint8),
            torch.zeros(self.compressed_scale_shape,
                        requires_grad=False,
                        device=device,
                        dtype=torch.float16),
        ) for _ in range(batch_size // micro_batch_size)]
    ...
    def compress(self, x, i_micro_batch):
        ...
        compressed_delta = _compress_topk_nbits_batch(
            ...
            scale_method=self.scale_method)
        # update cache
        delta = _decompress_topk_nbits_batch(*compressed_delta,
                                             bits=self.bits,
                                             k=self.k_delta,
                                             original_shape=self.activ_shape)
        x = last_x + delta
    ...
    def decompress(self, delta, i_micro_batch):
        ...
        last_x = cupy_to_tensor(self.cp_dec_buffers[i_micro_batch])
        # decompress delta
        delta = _decompress_topk_nbits_batch(*delta,
                                             bits=self.bits,
                                             k=self.k_delta,
                                             original_shape=self.activ_shape)
        # update cache
