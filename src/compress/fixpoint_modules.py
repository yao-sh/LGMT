...
class FixpointCompressor:
    ...
    def build_buffer(self,
        ...
        self.buffers = [(
            torch.zeros(
                (micro_batch_size, seq_length, embedding_dim * self.bits // 8),
                requires_grad=False,
                device=device,
                dtype=torch.uint8),
    ...
    def compress(self, x):
        if self.bits == 8:
            x, scale = compress_8bit(x,
                                     scale_method=self.scale_method,
                                     scale_dims=self.scale_dims)
            return x, scale
        elif self.bits == 4:
            x, scale = compress_4bit(x,
                                     scale_method=self.scale_method,
                                     scale_dims=self.scale_dims)
            return x, scale
        elif self.bits == 2:
            x, scale = compress_2bit(x,
                                     scale_method=self.scale_method,
                                     scale_dims=self.scale_dims)
            return x, scale
...
class FixpointFlexibleCompressor(FixpointCompressor):
    ...
    def compress(self, x):
        return compress_flexible_nbits(x,
                                       self.bits,
                                       scale_method=self.scale_method,
                                       scale_dims=self.scale_dims)
