...
def pack_uint8_tensor(x):
    if x.device != torch.device('cpu'):
        return cupy_to_tensor(cupy.packbits(tensor_to_cupy(x)))
    else:
        return torch.from_numpy(np.packbits(x.numpy()))


def unpack_uint8_tensor(x):
    if x.device != torch.device('cpu'):
        return cupy_to_tensor(cupy.unpackbits(tensor_to_cupy(x)))
    else:
        return torch.from_numpy(np.unpackbits(x.numpy()))
...
def pack_low_bit_tensor(x, bits):
    ...
    if x.device != torch.device('cpu'):
        ...
        y = cupy.packbits(
            cupy.unpackbits(tensor_to_cupy(x)).reshape(*x.shape, 8)[...,
                                                                    -bits:])
        y = cupy_to_tensor(y)
    else:
        y = np.packbits(
...
def unpack_low_bit_tensor(x, bits, original_shape):
    ...
    if x.device != torch.device('cpu'):
        y = cupy.packbits(
            cupy.pad(
                cupy.unpackbits(tensor_to_cupy(x))[:np.prod(original_shape) *
                                                   bits].reshape(-1, bits),
                ((0, 0), (8 - bits, 0))))
        ...
        y = np.packbits(
            np.pad(
                np.unpackbits(x.numpy())[:np.prod(original_shape) *
                                         bits].reshape(-1, bits),
                ((0, 0), (8 - bits, 0))))
