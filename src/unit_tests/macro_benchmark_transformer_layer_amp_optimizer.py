...
def benchmark_transformer_layer(args, device):
    ...
    for i in range(11):
        fake_batch = torch.zeros(batch_shape,
                                 requires_grad=True,
                                 dtype=torch.float16).to(device)
        fake_batch.uniform_(-0.05, 0.05)
        external_gradient = torch.full(batch_shape, 0.1,
                                       dtype=torch.float16).to(device)
        external_gradient.uniform_(-0.01, 0.01)

        print("Current memory allocated: {:2.3f} MB, peak memory: {:2.3f} MB".
              format(
        ...
        print("Iter ", i, "Current iter forward takes:",
              forward_start_event.elapsed_time(backward_start_event))
        print("Iter ", i, "Current iter backward takes:",
              backward_start_event.elapsed_time(optimizer_start_event))
        print("Iter ", i, "Current iter optimizer step takes:",
              optimizer_start_event.elapsed_time(end_event))
