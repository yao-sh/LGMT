...
def main():
    ...
    parser.add_argument('--fsdp-degree',
                        type=str,
                        default="recursive",
                        metavar='S',
    ...
    if args.fsdp_degree == 'simple':
        ...
        fsdp_config = dict(reshard_after_forward=True,
                           move_params_to_cpu=False,
                           mixed_precision=False,
                           flatten_parameters=False)
        with enable_wrap(wrapper_cls=FSDP, **fsdp_config):
            fsdp_model = auto_wrap(model,
    ...
    for i, data in enumerate(train_dataloader):
        ...
        print(
            "{}/{} Forward pass takes {:3.2f}s, loss: ".format(
                i % multi_iter, multi_iter, forward_time - cur_start_time),
            loss.item())
        print_cuda_memory(args, "FSDP forward iter is done", device)
        loss.backward()
        backward_time = time.time()
        print("{}/{} Backward pass takes {:3.2f}s".format(
            i % multi_iter, multi_iter, backward_time - forward_time))
