...
def load_decentralized_checkpoint(
    ...
    for i in range(n_stages):
        ...
        if i == 0:
            ...
            model.model.embed_tokens.weight.data[:] = _tmp[
                'embed_tokens.weight']
            ...
            for j in range(n_layer_per_stage):
                ...
                model.model.layers[i * n_layer_per_stage +
...
if __name__ == '__main__':
    ...
    config = LLaMAConfig.from_pretrained(
        '/root/fm/models/_root_fm_models_llama-7b')
    tokenizer = LLaMATokenizer.from_pretrained(
        '/root/fm/models/_root_fm_models_llama-7b')
    model = create_emtpy_llama(config)
    load_decentralized_checkpoint(
        model,
        '/root/Decentralized_FM_alpha/model_checkpoints/llama-7b-alpaca-fix/checkpoint_300',
        n_stages=2,
        n_layer_per_stage=16)
