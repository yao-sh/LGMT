...
def train_loop(args, pipe, device, train_data_loader, test_data_loader):
    ...
    input_ids = torch.zeros([args.batch_size, args.seq_length],
    ...
    if get_pipeline_parallel_rank() == 0 and dp_rank == 0:
        ...
        input_ids_mmap = np.memmap('pile_opt66_input_ids.mmap',
                                   dtype=np.int64,
                                   mode='r',
                                   shape=(n_samples, n_seq_length))
        hidden_states_mmap = np.memmap('pile_opt66_hidden_states.mmap',
                                       dtype=np.float16,
                                       mode='r',
                                       shape=(n_samples, n_seq_length,
                                              n_emb_size))
        ...
        while True:
            ...
            input_ids = torch.from_numpy(
                input_ids_mmap[pipe.global_step * args.batch_size *
                               dp_size:(pipe.global_step + 1) *
                               args.batch_size * dp_size])
            teacher_hidden_states = torch.from_numpy(
                hidden_states_mmap[pipe.global_step * args.batch_size *
                                   dp_size:(pipe.global_step + 1) *
                                   args.batch_size * dp_size])
            ...
            teacher_hidden_states_global = teacher_hidden_states.to(
                torch.float16).to(device)

            input_ids_list = input_ids_global.chunk(dp_size)
            teacher_hidden_states_list = teacher_hidden_states_global.chunk(
                dp_size)
