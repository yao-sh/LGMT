...
def train_loop(args, pipe, device, train_data_loader, test_data_loader):
    ...
    masks = torch.zeros([args.batch_size, args.seq_length],
    ...
    if get_pipeline_parallel_rank() == 0 and dp_rank == 0:
        ...
        for data in train_data_loader:
            ...
            masks_global = torch.ones_like(input_ids_global).to(
            ...
            current_iter_time = pipe.sgd_iter(input_ids,
                                              None,
                                              aux_input_data={'mask': masks})
        ...
        while True:
            ...
            labels[:,
                   1:][(1 - masks).bool()
                       [:, :-1]] = -100  # mask the next token of masked tokens
            compress.flag.FLAG_DISABLE_COMPRESSION = (
                pipe.global_step < args.train_warmup_steps)
            current_iter_time = pipe.sgd_iter(
                ...
                aux_input_data={'mask': masks})  # lm loss func
        ...
        while True:
            ...
            compress.flag.FLAG_DISABLE_COMPRESSION = (
                pipe.global_step < args.train_warmup_steps)
            current_iter_time = pipe.sgd_iter(None,
                                              None,
                                              aux_input_data={'mask': masks})
...
def main():
    ...
    # num layer globally
    args.max_layers = config.num_layers if hasattr(
        config, 'num_layers') else config.n_layer
