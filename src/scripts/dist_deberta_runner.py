...
def main():
    ...
    if args.task_name == 'sst2':
        train_data_loader = get_sst2_data_loader(args,
                                                 tokenizer,
                                                 data_split='train')
        test_data_loader = get_sst2_data_loader(args,
                                                tokenizer,
                                                data_split='validation')
        config.num_labels = 2
    elif args.task_name == 'mrpc':
        train_data_loader = get_mrpc_data_loader(args,
                                                 tokenizer,
                                                 data_split='train')
        test_data_loader = get_mrpc_data_loader(args,
                                                tokenizer,
                                                data_split='validation')
        config.num_labels = 2
    elif args.task_name == 'cola':
        train_data_loader = get_cola_data_loader(args,
                                                 tokenizer,
                                                 data_split='train')
        test_data_loader = get_cola_data_loader(args,
                                                tokenizer,
                                                data_split='validation')
        config.num_labels = 2
    elif args.task_name == 'qqp':
        train_data_loader = get_qqp_data_loader(args,
                                                tokenizer,
                                                data_split='train')
        test_data_loader = get_qqp_data_loader(args,
                                               tokenizer,
                                               data_split='validation')
        config.num_labels = 2
    elif args.task_name == 'qnli':
        train_data_loader = get_qnli_data_loader(args,
                                                 tokenizer,
                                                 data_split='train')
        test_data_loader = get_qnli_data_loader(args,
                                                tokenizer,
                                                data_split='validation')
        config.num_labels = 2
    else:
    ...
    if args.load_pretrained_model:
        if get_pipeline_parallel_rank() == 0:
            ...
            if hasattr(pipe.model.encoder,
                       'conv') and pipe.model.encoder.conv is not None:
