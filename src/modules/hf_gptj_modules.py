...
class GPTJMLP(_GPTJMLP):
    ...
    def __init__(self,
                 intermediate_size,
                 config,
                 device='cpu'):  # in MLP: intermediate_size= 4 * embed_dim
    ...
    def _attn(
        ...
        if prefix_mask is not None:
            ...
            causal_mask = causal_mask.repeat(bsz, 1, 1,
                                             1)  # (bsz, 1, src_len, tgt_len)
            causal_mask = causal_mask.permute(0, 3, 1,
                                              2)  # (bsz, tgt_len, 1, src_len)
            causal_mask[prefix_mask.bool()] = 1
            causal_mask = causal_mask.permute(0, 2, 3,
                                              1)  # (bsz, 1, src_len, tgt_len)
    ...
    def forward(self,
        ...
        if self.training:
            ...
            if self.use_checkpoint:
                ...
                x = checkpoint(self.block_forward, x, attention_mask,
                               prefix_masks)
            else:
