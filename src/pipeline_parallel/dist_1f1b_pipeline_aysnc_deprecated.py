...
class Pipe1F1BAsync:
    ...
    def __init__(self, args, vocab_size, num_classes, device):
        ...
        self.torch_forward_recv_stream = torch.cuda.Stream(device=device,
                                                           priority=-1)
        self.torch_backward_recv_stream = torch.cuda.Stream(device=device,
                                                            priority=-1)
        self.torch_forward_send_stream = torch.cuda.Stream(device=device,
                                                           priority=-1)
        self.torch_backward_send_stream = torch.cuda.Stream(device=device,
                                                            priority=-1)

        self.forward_recv_ready_events = [
            torch.cuda.Event(enable_timing=self.enable_tidy_profiling,
                             blocking=False)
        ...
        if self.enable_tidy_profiling:
            ...
            self.optimizer_start_event = torch.cuda.Event(enable_timing=True,
                                                          blocking=False)
            self.optimizer_end_event = torch.cuda.Event(enable_timing=True,
                                                        blocking=False)
        ...
        if args.rank == 0:
            ...
            self.input_micro_batches = [
                torch.zeros((self.micro_batch_size, self.seq_length,
                             self.embedding_dim),
                            requires_grad=True,
                            device=self.device)
                for _ in range(self.micro_batch_num)
        ...
        self.output_micro_batches = [
            torch.zeros(
                ...
                requires_grad=True,
                device=self.device) for _ in range(self.micro_batch_num)
        ]
    ...
    def profile_mark_forward_recv_start(self, i):
        if self.enable_tidy_profiling:
            self.torch_forward_recv_stream.record_event(
                self.forward_recv_start_events[i])

    def profile_mark_forward_send_start(self, i):
        if self.enable_tidy_profiling:
            self.torch_forward_send_stream.record_event(
    ...
    def profile_mark_backward_recv_start(self, i):
        if self.enable_tidy_profiling:
            self.torch_backward_recv_stream.record_event(
                self.backward_recv_start_events[i])

    def profile_mark_backward_send_start(self, i):
        if self.enable_tidy_profiling:
            self.torch_backward_send_stream.record_event(
    ...
    def forward_micro_batch(self, forward_index):
        if self.rank == 0:  # Only send output to next node, do not receive
            with torch.cuda.stream(self.torch_comp_stream):
                ...
                self.output_micro_batches[forward_index] = self.model(
                    self.input_micro_batches[forward_index])
                self.torch_comp_stream.record_event(
                    self.forward_comp_ready_events[forward_index])
            with torch.cuda.stream(self.torch_forward_send_stream):
                cupy_forward_send_stream = cupy.cuda.ExternalStream(
                    self.torch_forward_send_stream.cuda_stream)
                self.torch_forward_send_stream.wait_event(
                    self.forward_comp_ready_events[forward_index])
                self.profile_mark_forward_send_start(forward_index)
                self.comm.send(self.output_micro_batches[forward_index].data,
            ...
            with torch.cuda.stream(self.torch_forward_recv_stream):
                cupy_forward_recv_stream = cupy.cuda.ExternalStream(
                    self.torch_forward_recv_stream.cuda_stream)
                self.profile_mark_forward_recv_start(forward_index)
                self.comm.recv(self.input_micro_batches[forward_index],
                               src=self.pre_node_rank,
                               stream=cupy_forward_recv_stream)
                self.torch_forward_recv_stream.record_event(
                    self.forward_recv_ready_events[forward_index])
            with torch.cuda.stream(self.torch_comp_stream):
    ...
    def backward_micro_batch(self,
                             backward_index,
                             target_as_micro_batch=None,
                             loss_func=None):
        if self.rank == self.world_size - 1:  # only send grad back to last node, do not receive
            with torch.cuda.stream(self.torch_comp_stream):
                ...
                loss = loss_func(
                    input=self.output_micro_batches[backward_index],
                    target=target_as_micro_batch)
                loss.backward()
                self.torch_comp_stream.record_event(
                    self.backward_comp_ready_events[backward_index])
            with torch.cuda.stream(self.torch_backward_send_stream):
                cupy_backward_send_stream = cupy.cuda.ExternalStream(
                    self.torch_backward_send_stream.cuda_stream)
                self.torch_backward_send_stream.wait_event(
                    self.backward_comp_ready_events[backward_index])
                self.profile_mark_backward_send_start(backward_index)
                self.comm.send(self.input_micro_batches[backward_index].grad,
            ...
            with torch.cuda.stream(self.torch_backward_recv_stream):
                cupy_backward_recv_stream = cupy.cuda.ExternalStream(
                    self.torch_backward_recv_stream.cuda_stream)
                self.profile_mark_backward_recv_start(backward_index)
                self.comm.recv(self.output_micro_batches_grad[backward_index],
                               src=self.post_node_rank,
                               stream=cupy_backward_recv_stream)
                self.torch_backward_recv_stream.record_event(
                    self.backward_recv_ready_events[backward_index])
            with torch.cuda.stream(self.torch_comp_stream):
                self.torch_comp_stream.wait_event(
                    self.backward_recv_ready_events[backward_index])
                self.profile_mark_backward_comp_start(backward_index)
    ...
    def forward_backward_stages(self,
                                input_data=None,
                                target=None,
        ...
        if self.rank == 0:
            ...
            self.input_micro_batches = torch.chunk(input_data,
                                                   self.micro_batch_num,
                                                   dim=0)
            target_as_micro_batches = [
                None for _ in range(self.micro_batch_num)
            ]
        elif self.rank == self.world_size - 1:
            assert (target is not None)
            target_as_micro_batches = torch.chunk(target,
                                                  self.micro_batch_num,
                                                  dim=0)
        else:
        ...
        while forward_i < self.micro_batch_num:
            ...
            self.backward_micro_batch(
                backward_index=backward_i,
                target_as_micro_batch=target_as_micro_batches[backward_i],
                loss_func=loss_func)
            forward_i += 1
            backward_i += 1
    ...
    def profiling_optimizer_step(self):
        ...
        optimizer_log = {
            ...
            "ts": self.optimizer_start_time_stamp,
            "dur": optimizer_slot,
            "cname": "bad"
        }
        print(optimizer_log)
    ...
    def sgd_iter(self, input_=None, target=None):
        ...
        print("Rank {} node 1f1b iteration takes {:3.2f}s".format(
            self.rank, iter_time))
        return iter_time
