...
def create_optimizer(model,
    ...
    optimizer_grouped_parameters = [{
        "params": [
            ...
            if n in decay_parameters and 'upscale_last' not in n
        ],
        "weight_decay":
        weight_decay,
    }, {
        "params":
        [p for n, p in model.named_parameters() if 'upscale_last' in n],
        "weight_decay":
        weight_decay,
        "lr":
        1e-2,
        ...
        "params": [
            p for n, p in model.named_parameters()
            if n not in decay_parameters and 'upscale_last' not in n
        ],
        "weight_decay":
        0.0,
    }]
...
class GpipeAsyncDistill:
    ...
    def __init__(self,
        ...
        if do_train:
            if self.use_fp16:
                tmp_optimizer = create_optimizer(self.model,
    ...
    def _compute_micro_batch_size(self):
        ...
        print("=======Number of micro-batches: {}.".format(
    ...
    def forward_stage(self, input_data=None, aux_input_data=None):
        ...
        if aux_input_data is not None:
            for k in aux_input_data:
                aux_input_data[k] = torch.chunk(aux_input_data[k],
                                                self.micro_batch_num,
                                                dim=0)
        else:
            aux_input_data = {}
        ...
        if self.pp_rank == self.pipeline_group_size - 1:
            if input_data is not None:
                input_ids_micro_batches = torch.chunk(input_data,
                                                      self.micro_batch_num,
                                                      dim=0)
            else:
        ...
        for i in range(self.micro_batch_num):
            if self.pp_rank == 0:  # Only send output to next node, do not receive
                with torch.cuda.stream(self.torch_comp_stream):
                    ...
                    current_micro_output = self.model(
                        self.input_micro_batches[i], **{
                            k: v[i]
                            for k, v in aux_input_data.items()
                        })
                    self.torch_comp_stream.record_event(
                ...
                with torch.cuda.stream(self.torch_recv_stream):
                    ...
                    self.comm.recv(self.input_micro_batches[i],
        ...
        for i in range(self.micro_batch_num):
            if self.pp_rank == self.pipeline_group_size - 1:  # only send grad back to last node, do not receive
                with torch.cuda.stream(self.torch_comp_stream) as st:
                    ...
                    loss = loss_func(input=cached_output_micro_batches[i][0],
                ...
                with torch.cuda.stream(self.torch_send_stream):
                    ...
                    if self.use_fp16 and self.use_dynamic_scale:
                        self.input_micro_batches[i].grad.copy_(
                            self.optimizer.unscale(
                                self.input_micro_batches[i].grad))
                    self.comm.send(self.input_micro_batches[i].grad,
                ...
                with torch.cuda.stream(self.torch_recv_stream):
                    ...
                    self.comm.recv(self.output_micro_batches_grad[i],
                                   src=self.post_node_rank,
                                   stream=cupy_recv_stream)
                    if self.use_fp16 and self.use_dynamic_scale:
                        self.output_micro_batches_grad[i].copy_(
                            self.optimizer.scale(
            ...
            wandb.log(
                {
                    ...
                    'distill_loss':
                    sum(tr_distill_loss) / len(tr_distill_loss),
    ...
    def sgd_iter(self,
        ...
        print("Rank {} node forward pass {}/{} takes {:3.2f}s".format(
            self.global_rank, step, self.gradient_accumulate_step,
            forward_slot))
        ...
        print("Rank {} node backward pass {}/{} takes {:3.2f}s".format(
            self.global_rank, step, self.gradient_accumulate_step,
        ...
        for i in range(self.micro_batch_num):
            if self.pp_rank == 0:  # Only send output to next node, do not receive
                ...
                with torch.cuda.stream(self.torch_comp_stream):
                    ...
                    current_micro_output = pred_func(current_micro_output,
                                                     labels[i])
                    self.torch_comp_stream.record_event(
                        self.forward_comp_ready_events[i])
            else:  # receive, compute, and send
    ...
    def infer_iter(self,
                   ...
                   metrics=None,
                   aux_input_data=None,
                   pred_func=lambda x, y: x.argmax(-1)):
        self.comm.barrier()
        torch.cuda.synchronize()
        with torch.no_grad():
