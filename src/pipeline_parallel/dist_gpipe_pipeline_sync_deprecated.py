...
class GpipeSync:
    ...
    def forward_stage(self, input_data=None):
        ...
        if self.rank == 0:
            ...
            input_micro_batches = torch.chunk(input_data,
    ...
    def backward_stage(self,
                       cached_input_micro_batches: List[torch.Tensor],
        ...
        for micro_batch_index in range(self.micro_batch_num):
            ...
            if self.post_node_rank != -1:  # <=> rank != world_size - 1
                if type(self.comm) is NCCLCommunicator:
                    ...
                    output_micro_batch_grad = torch.zeros(
                        (self.seq_length, self.micro_batch_size,
                         self.embedding_size))
                self.comm.recv(output_micro_batch_grad,
                               src=self.post_node_rank)
                cached_output_micro_batches[micro_batch_index].to(
                    self.device).backward(
                        gradient=output_micro_batch_grad.to(self.device))
            else:
                loss = torch.nn.functional.cross_entropy(
                    input=cached_output_micro_batches[micro_batch_index].to(
                        self.device),
                    target=target_as_micro_batches[micro_batch_index].to(
                        self.device))
                loss.backward()
            if self.pre_node_rank != -1:
                if type(self.comm) is NCCLCommunicator:
                    self.comm.send(
                        cached_input_micro_batches[micro_batch_index].grad,
                        dst=self.pre_node_rank)
                else:
                    self.comm.send(
                        cached_input_micro_batches[micro_batch_index].cpu(
                        ).grad,
                        dst=self.pre_node_rank)
